<?php
/**
 * IMPORTAÇÃO APENAS SETEMBRO 2025 - CMED
 * Dados mais atualizados com preços completos
 */
...
echo "╔════════════════════════════════════════════════════════════╗\n";
echo "║  IMPORTAÇÃO CMED - SETEMBRO 2025 (MAIS RECENTE)           ║\n";
echo "╚════════════════════════════════════════════════════════════╝\n\n";
...
$arquivo = 'CMED Setembro 25 - Modificada.xlsx';
...
try {
    ...
    for ($r = 1; $r <= 10; $r++) {
        ...
        // Procurar por PRODUTO/APRESENTAÇÃO
        $temProduto = false;
    ...
    foreach ($cabecalho as $idx => $nome) {
        ...
        if (preg_match('/PMC\s*0/', $nomeUpper) || preg_match('/PMC\s*SEM/', $nomeUpper)) {
            $map['pmc_0'] = $idx;
        }
        ...
        // Se não achou PMC, procurar PF (Preço Fábrica)
        if (!isset($map['pmc_0']) && preg_match('/PF\s*0/', $nomeUpper)) {
            $map['pf_0'] = $idx;
        }
    }
    ...
    echo "🚀 Iniciando importação de SETEMBRO 2025...\n\n";
    ...
    for ($r = $linhaHeader + 1; $r <= $highestRow; $r++) {
        ...
        // Extrair preços PMC (ou PF se PMC não existir)
        $pmc0 = isset($map['pmc_0']) ? ($linha[$map['pmc_0']] ?? null) : (isset($map['pf_0']) ? ($linha[$map['pf_0']] ?? null) : null);
    ...
    echo "\n";
    echo "╔════════════════════════════════════════════════════════════╗\n";
    echo "║  RESULTADO DA IMPORTAÇÃO - SETEMBRO 2025                   ║\n";
    echo "╚════════════════════════════════════════════════════════════╝\n\n";
    ...
    echo "✅ Importação de SETEMBRO 2025 concluída com sucesso!\n";
    echo "💊 Preços atualizados e prontos para uso!\n";
